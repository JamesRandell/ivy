...
class Ivy_Database extends Ivy_Dictionary {
...
//echo $query.'________<br>';
		...
		$query = $this->pagination($query, array(	'from'	=>	$this->query['from'],
													'where'	=>	$where,
													'databaseSpec' => array (
...
//echo '</pre>';
		...
		echo $this->schema['tableSpec']['name'] . '.' . $this->schema['tableSpec']['pk'][0];
...
echo '</pre>';
		...
		$query = 'UPDATE '.$this->schema['tableSpec']['name'].' SET '.$string.' WHERE '.$this->schema['tableSpec']['pk'][0]." =
			 '".$dataArray[$this->schema['tableSpec']['pk'][0]]."'";
...
//echo '</pre>';
	...
	private function join ()
	{
		...
		if (is_array($join = $validation->sJoin('join'))) {
			...
			foreach ($join as $field => $joinSpec) {
				...
				if (isset($this->schema['fieldSpec'][$field]['join']['fields'])) {
					$tempArray = array ();
					$findArray = array ();
					$replaceArray = array ();
					
					foreach ($this->schema['fieldSpec'][$field]['join']['fields'] as $joinField) {
				...
				$fieldOriginal = $field;
				foreach ($array['fieldSpec'] as $field => $data) {
					$array['fieldSpec'][$joinSpec['table'] . '.' . $field] = $data;
				...
				$this->query['from'] .=  			
						$joinType . $joinSpec['table'] . 
						' ON ' . $fieldOriginal . 
							' = ' . $joinSpec['table'] . '.' . $joinSpec['pk'];
		...
		if (is_array($join = $validation->sJoin('replace'))) {
			...
			foreach ($join as $field => $joinSpec) {
				...
				#$r = rtrim($r, "||''");
					...
					$this->query['field'][$this->schema['tableSpec']['name']]['_' . $field] = $this->schema['tableSpec']['name'] . '.' . $fieldT . ' as "_' . ereg_replace('"', '', $field) . '"';
					...
					if (isset($this->schema['fieldSpec'][$field]['replace']['where'])) {
						$this->query['field'][$this->schema['tableSpec']['name']][$field] = '(select ' . $r . ' from ' . $joinSpec['table'] . ' where ' . $joinSpec['table'] . '.' . $joinSpec['key'] 
							. ' = ' . $this->schema['tableSpec']['name'] . '.' . $field . ' AND ' . $this->schema['fieldSpec'][$field]['replace']['where'] . ') as "' . ereg_replace('"', '', $field) . '"';
					} else {
						$this->query['field'][$this->schema['tableSpec']['name']][$field] = '(select ' . $r . ' from ' . $joinSpec['table'] . ' where ' . $joinSpec['table'] . '.' . $joinSpec['key'] 
							. ' = ' . $this->schema['tableSpec']['name'] . '.' . $fieldT . ') as "' . ereg_replace('"', '', $field) . '"';
					}
				#}
			}
		}
	...
	{		
		...
		if ($numRows <= $rowsPerPage) {
			...
			$parameterArray['order'] = (($this->order[1]) ? 
				' ORDER BY ' . $this->order : 'ORDER BY ' . $parameterArray['pk']);
